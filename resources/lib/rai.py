    ...
    def getChannels(self, _url, channel_url, fetched_channels):
        ...
        if 'dirette' in data:
            for entry in data['dirette']:
                if any(d.get('title', None) == entry['channel'] for d in fetched_channels):
                    fetched_channel = next((x for x in fetched_channels if x.get('title', None) == entry['channel']), None)
                    icon = self.getThumbnailUrl(entry["transparent-icon"])
                    title = entry["channel"]
                    url = utils.get_url(_url, action='play', type='live', channel=self.__class__.__name__,  callSign = entry["video"]["contentUrl"])
